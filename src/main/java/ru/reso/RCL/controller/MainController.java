...
import ru.reso.RCL.service.ResoComparator;
import ru.reso.RCL.model.CalcID;
...
public class MainController {
    ...
    public String submit(@ModelAttribute("CalcId") CalcID calcID, BindingResult result, ModelMap model) {
        ...
        if ((calcID.getType() == null) || (calcID.getCalcId() == null) || (calcID.getCalcIdSecond() == null)) { // проверка чтобы был выбран ТИП и calcid (оба) не были пустыми
            return "error"; // если пустые, то переадресовываем на страничку ошибки.
        } else {
            ...
            switch (calcID.getType()) {
                case 1:
                    ...
                    LinkedHashMap<String, ComparableValues> fullCoefsCompareResult = resoComparator.wsCoeffCompare(calcID.getCalcId(), calcID.getCalcIdSecond(), 1);
                ...
                case 2: // отображдение только несоответствий
                    ...
                    LinkedHashMap<String, ComparableValues> fullCoefsCompareResultButOnlyDifferences = resoComparator.wsCoeffCompare(calcID.getCalcId(), calcID.getCalcIdSecond(), 2);
